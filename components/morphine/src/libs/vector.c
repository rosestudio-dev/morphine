...
static void dynamic(morphine_coroutine_t U) {
    maux_nb_function(U)
        maux_nb_init();
        ml_size size = 0;
        if (mapi_args(U) != 0) {
            ...
            mapi_push_arg(U, 0);
            size = mapi_get_size(U, NULL);
        }

        mapi_push_vector_dyn(U, size);
        maux_nb_return();
    maux_nb_end
}
...
static void trim(morphine_coroutine_t U) {
    maux_nb_function(U)
        ...
        mapi_push_vector_fix(U, from <= to ? to - from : 0);
...
static maux_construct_element_t elements[] = {
    ...
    MAUX_CONSTRUCT_FUNCTION("dynamic", dynamic),
