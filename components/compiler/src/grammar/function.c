...
void match_function(struct matcher *M) {
    matcher_consume(M, symbol_predef_word(MCLTPW_fun));

    if (matcher_match(M, symbol_predef_word(MCLTPW_recursive))) {
    ...
    if (matcher_look(M, symbol_operator(MCLTOP_LT))) {
        R.open_symbol = symbol_operator(MCLTOP_LT);
        R.close_symbol = symbol_operator(MCLTOP_GT);

        if (argument_matcher_init(&R, 0)) {
            if (!matcher_match(M, symbol_predef_word(MCLTPW_auto))) {
                do {
                    matcher_consume(M, symbol_word);
    ...
    R.open_symbol = symbol_operator(MCLTOP_LPAREN);
    R.close_symbol = symbol_operator(MCLTOP_RPAREN);
    ...
    if (matcher_match(M, symbol_predef_word(MCLTPW_static))) {
        if (argument_matcher_init(&R, 0)) {
            do {
                matcher_consume(M, symbol_word);
            } while (argument_matcher_next(&R));
...
static struct ast_function *create_function(morphine_coroutine_t U, struct ast *A, struct elements *E) {
    ...
    if (elements_look(E, 1, symbol_predef_word(MCLTPW_recursive))) {
        recursive = true;
        anonymous = false;
    ...
    size_t closures = 0;
    if (elements_look(E, start_pos, symbol_operator(MCLTOP_LT))) {
        R.open_symbol = symbol_operator(MCLTOP_LT);
        R.close_symbol = symbol_operator(MCLTOP_GT);

        if (argument_matcher_init(&R, start_pos)) {
            if (argument_matcher_match(&R, symbol_predef_word(MCLTPW_auto))) {
                auto_closure = true;
            } else {
                do {
    ...
    R.consume_open = false;
    size_t statics = 0;
    if (elements_look(E, R.pos, symbol_predef_word(MCLTPW_static))) {
    ...
    if (elements_look(E, R.pos, symbol_operator(MCLTOP_EQ))) {
