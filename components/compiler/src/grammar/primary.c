...
void match_primary(struct matcher *M) {
    ...
    if (matcher_look(M, symbol_operator(MCLTOP_LBRACKET)) || matcher_is_reduced(M, REDUCE_TYPE_VECTOR)) {
    ...
    if (matcher_look(M, symbol_predef_word(MCLTPW_if)) || matcher_is_reduced(M, REDUCE_TYPE_EXPRESSION_IF)) {
        matcher_reduce(M, REDUCE_TYPE_EXPRESSION_IF);
        return;
    }

    if (matcher_match(M, symbol_predef_word(MCLTPW_do))) {
...
struct ast_node *assemble_primary(morphine_coroutine_t U, struct ast *A, struct elements *E) {
    ...
    if (matcher_symbol(symbol_predef_word(MCLTPW_do), watch_token)) {
