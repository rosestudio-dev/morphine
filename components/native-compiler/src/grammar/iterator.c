...
void match_iterator(struct matcher *M) {
    matcher_consume(M, symbol_predef_word(TPW_iterator));
    matcher_consume(M, symbol_operator(TOP_LPAREN));

    match_decompose(M, true);
    matcher_consume(M, symbol_predef_word(TPW_in));
...
struct ast_node *assemble_iterator(morphine_coroutine_t U, struct elements *E) {
    size_t index = 0;
    size_t size = size_decompose(U, E, true, 2, &index);

    struct statement_iterator *result = ast_create_statement_iterator(U, elements_line(E, 0), size);
    ...
    if (size == 0) {
        insert_decompose(U, E, true, 2, &result->name, NULL, NULL);
    } else {
        insert_decompose(U, E, true, 2, result->multi.names, NULL, result->multi.keys);
    }
